...
export async function fetchTasks(token: string): Promise<Task[]> {
  const res = await apiFetch("/api/tasks", { headers: headers(token) });
...
export async function apiLogSession(
  ...
  try {
    ...
    // fire-and-forget — losing a log is non-critical
  }
}
...
export async function fetchWeekStudyDays(token: string, from?: string): Promise<string[]> {
  const path = from
    ? `/api/sessions/week?from=${encodeURIComponent(from)}`
    : "/api/sessions/week";
  const res = await apiFetch(path, { headers: headers(token) });
...
export async function fetchCurrentStreak(token: string): Promise<StreakInfo> {
  let res: Response;
  try {
    res = await apiFetch("/api/sessions/streak", { headers: headers(token) });
  } catch {
    return { streak: 0, lastActiveDate: null, activeToday: false };
  }
...
export async function fetchWeeklySummary(token: string): Promise<WeeklySummary | null> {
  let res: Response;
  try {
    res = await apiFetch("/api/sessions/weekly-summary", { headers: headers(token) });
  } catch {
    return null;
  }
...
export async function fetchDaySummary(token: string, date: string): Promise<DaySummary> {
  let res: Response;
  try {
    res = await apiFetch(`/api/day-summary?date=${encodeURIComponent(date)}`, { headers: headers(token) });
  } catch {
...
export async function fetchResources(token: string): Promise<Resource[]> {
  const res = await apiFetch("/api/resources", { headers: headers(token) });
...
export async function fetchNotes(token: string): Promise<Note[]> {
  const res = await apiFetch("/api/notes", { headers: headers(token) });
...
export async function fetchNote(token: string, id: string): Promise<Note> {
  const res = await apiFetch(`/api/notes/${id}`, { headers: headers(token) });
...
export async function apiGeneratePlan(
  ...
  const res = await apiFetch("/api/tasks/generate-plan", {
    method: "POST",
    headers: headers(token),
    body: JSON.stringify(data),
  });
...
export async function apiConfirmPlan(
  ...
  const res = await apiFetch("/api/tasks/confirm-plan", {
    method: "POST",
    headers: headers(token),
...
export async function fetchGCalStatus(token: string): Promise<{ connected: boolean; gcalEmail?: string }> {
  let res: Response;
  try {
    res = await apiFetch("/api/gcal/status", { headers: headers(token) });
  } catch {
...
export async function fetchGCalAuthUrl(token: string): Promise<string | null> {
  let res: Response;
  try {
    res = await apiFetch("/api/gcal/auth-url", { headers: headers(token) });
  } catch {
    return null;
  }
  if (!res.ok) return null;
...
export async function fetchGCalEvents(token: string, days = 7): Promise<GCalEvent[]> {
  let res: Response;
  try {
    res = await apiFetch(`/api/gcal/events?days=${days}`, { headers: headers(token) });
  } catch {
    return [];
  }
  if (!res.ok) return [];
...
export async function apiGCalDisconnect(token: string): Promise<void> {
  try {
    await apiFetch("/api/gcal/disconnect", { method: "DELETE", headers: headers(token) });
  } catch {
...
export async function apiGCalAddEvent(
  ...
  let res: Response;
  try {
    res = await apiFetch("/api/gcal/add-event", {
      method: "POST",
      headers: headers(token),
      body: JSON.stringify(data),
    });
