...
export async function ensureRoomsSchema() {
	await sql`
		create table if not exists public.study_rooms (
			...
			subject     text,
			description text,
			room_mode   text not null default 'pomodoro',
	...
	await sql`
		create table if not exists public.study_room_members (
			...
			unique (room_id, clerk_id)
		)
	`;
	await sql`
		create table if not exists public.study_room_tasks (
			id         uuid primary key default gen_random_uuid(),
			room_id    uuid not null references public.study_rooms(id) on delete cascade,
			clerk_id   text not null,
			title      text not null,
	...
	`;
	await sql`alter table public.study_rooms enable row level security`;
