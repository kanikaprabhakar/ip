...
function isInvalidGrant(err) {
	...
	return /invalid_grant|expired|revoked/i.test(`${msg} ${code} ${desc} ${cause}`);
}

async function clearBrokenConnection(clerkId) {
	await sql`
...
export async function getEvents(req, res) {
	...
	try {
		...
		if (isInvalidGrant(err)) {
			console.warn("GET /api/gcal/events: refresh token expired or revoked");
			await clearBrokenConnection(clerkId);
			return res.status(401).json({ error: "Google Calendar connection expired. Please reconnect.", reauthRequired: true });
		}
...
export async function addEvent(req, res) {
	...
	try {
		...
		if (isInvalidGrant(err)) {
			console.warn("POST /api/gcal/add-event: refresh token expired or revoked");
			await clearBrokenConnection(clerkId);
			return res.status(401).json({ error: "Google Calendar connection expired. Please reconnect.", reauthRequired: true });
		}
