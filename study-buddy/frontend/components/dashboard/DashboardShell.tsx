    ...
    getToken().then(async (token) => {
      ...
      if (connected) {
        ...
        if (!cancelled && events.length === 0) {
          const latest = await fetchGCalStatus(token);
          if (!latest.connected) {
    ...
    <main className="relative min-h-screen overflow-x-hidden animate-gradient-bg">
      ...
      <div className="relative z-10 mx-auto w-full max-w-5xl px-4 pb-16 sm:px-6">
        ...
        <div className="grid gap-5 md:grid-cols-2">
          ...
          {/* Study rooms */}
          <DashCard title="Study Rooms" href="/rooms">
            <div className="flex items-center gap-4">
              <div className="relative flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl" style={{ background: "linear-gradient(135deg,#4D3449,#CB438B)" }}>
                <span className="font-display text-sm font-bold italic text-white">Grp</span>
              </div>
              <div>
                <p className="text-sm font-semibold text-fg-primary">Create a live room</p>
                <p className="text-xs text-fg-secondary">Share an invite code, add room tasks, and chat while you study.</p>
              </div>
            </div>
            <Link
              href="/rooms"
              onClick={(e) => e.stopPropagation()}
              className="relative z-10 mt-4 inline-flex items-center gap-2 rounded-2xl px-5 py-2.5 text-sm font-bold text-white shadow-lg transition-all hover:scale-105"
              style={{ background: "linear-gradient(135deg,#CB438B 0%,#BF3556 100%)" }}
            >
              Open rooms
            </Link>
...
const NAV_PAGES = [
  ...
  { href: "/rooms",     label: "Study Rooms", img: "/images/13.png", sub: "Live study space" },
